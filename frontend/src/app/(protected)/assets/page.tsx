...
const STATUSES = [
  ...
  },
  {
    value: 'disposed',
    label: 'Disposed',
    color: 'text-muted-foreground/50 bg-muted/30 border-border/50',
  },
];
...
function warrantyBadge(expiry: string | null) {
  ...
  if (days < 0)
    return (
      <span className="text-[10px] px-1.5 py-0.5 rounded border bg-red-500/10 border-red-500/20 text-red-400">
        Warranty expired
      </span>
    );
  if (days <= 30)
...
function InlineStatusDropdown({
  ...
  async function pick(newStatus: string) {
    ...
    try {
      await api.post(
        '/api/assets/bulk',
        { asset_ids: [assetId], action: 'status_change', status: newStatus },
        orgId
      );
      onChanged();
    } finally {
...
function BulkBar({
  ...
  async function applyStatus() {
    ...
    try {
      await api.post(
        '/api/assets/bulk',
        { asset_ids: selected, action: 'status_change', status: newStatus },
        orgId
      );
      onDone();
    } finally {
  ...
  async function bulkDelete() {
    ...
    try {
      await api.post(
        '/api/assets/bulk',
        { asset_ids: selected, action: 'delete' },
        orgId
      );
      onDone();
    } finally {
...
function InsightsPanel({ orgId }: { orgId: string }) {
  ...
  const handleAct = async (insight: Insight) => {
    ...
    try {
      ...
      else if (res.license_id)
        router.push(`/assets/licenses/${res.license_id}`);
  ...
  const lastRun = data?.last_run?.last_run_at
    ? new Date(data.last_run.last_run_at).toLocaleTimeString(undefined, {
        hour: '2-digit',
        minute: '2-digit',
      })
    : null;
              ...
              className={cn(
                ...
                ins.severity === 'critical'
                  ...
                  : ins.severity === 'warning'
                    ...
                    : ins.severity === 'notice'
                      ? 'border-l-blue-500 bg-blue-500/3'
...
interface AssetDashboard {
  ...
  totals?: { total_assets?: number } | null;
  warranty_alerts?: unknown[];
}
...
export default function AssetsPage() {
  ...
  const { data, isLoading, mutate } = useSWR(
    ...
    ([url, oid]) =>
      api.get<{ assets: AssetSummary[]; total: number; pages: number }>(
        url,
        oid
      )
  );
  ...
  const { data: dashboard } = useSWR(
    ...
    ([url, oid]) => api.get<AssetDashboard>(url, oid)
  );
  ...
  const handleImport = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        alert(
          `Imported ${result.created} assets. ${result.errors?.length ?? 0} errors.`
        );
        mutate();
      } finally {
        setImporting(false);
  ...
  const statSummary = dashboard
    ? [
        {
          label: 'Active',
          value: dashboard.by_status?.active ?? 0,
          color: 'text-emerald-400',
        },
        {
          label: 'Deployed',
          value: dashboard.by_status?.deployed ?? 0,
          color: 'text-blue-400',
        },
        {
          label: 'In Repair',
          value: dashboard.by_status?.in_repair ?? 0,
          color: 'text-amber-400',
        },
        {
          label: 'Total',
          value: dashboard.totals?.total_assets ?? 0,
        ...
        {warrantyAlerts.length > 0 && (
          <div className="flex items-center gap-3 p-3 rounded-xl border border-amber-500/20 bg-amber-500/5">
            ...
            <p className="text-xs text-amber-400">
              <strong>{warrantyAlerts.length}</strong> asset
              {warrantyAlerts.length > 1 ? 's' : ''} with warranty expiring
              within 90 days.{' '}
              <button
                onClick={() => setStatus('')}
                  ...
                  >
                    ...
                    <td className="px-4 py-3 hidden lg:table-cell">
                      {asset.assigned_email ? (
                        <div>
                          <p className="text-xs">
                            {asset.assigned_name || asset.assigned_email}
                          </p>
