...
function AssignModal({
  ...
  const lookupAsset = async () => {
    ...
    try {
      ...
      const match = res.assets.find(
        a => a.asset_tag.toLowerCase() === assetTag.toLowerCase()
      );
      if (match) {
        setAssetId(match.id);
        setErr('');
      } else {
        setErr('Asset tag not found');
        setAssetId('');
      }
    } catch {
  ...
  const submit = async () => {
    if (mode === 'user' && !userId) {
      setErr('Look up a user first');
      return;
    }
    if (mode === 'asset' && !assetId) {
      setErr('Look up an asset first');
      return;
    }
    setSaving(true);
    setErr('');
    try {
      await api.post(
        `/api/assets/licenses/${licenseId}/assign`,
        {
      ...
      setErr(errorMessage(e, 'Failed to assign seat'));
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-sm shadow-2xl space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          <strong className="text-foreground">{productName}</strong> and revoke
          all active seat assignments. This cannot be undone.
        </p>
...
export default function LicenseDetailPage({
  ...
  const handleRevoke = async (assignmentId: string) => {
    ...
    try {
      await api.delete(
        `/api/assets/licenses/${licenseId}/assignments/${assignmentId}`,
        orgId
      );
      mutate();
    } finally {
  ...
  const daysLeft = lic.expiry_date
    ? Math.round(
        (new Date(lic.expiry_date).getTime() - Date.now()) / 86_400_000
      )
    : null;

  const expStatus = lic.is_expired
    ? 'expired'
  ...
  const seatPct = lic.seat_count
    ? Math.min(100, Math.round((lic.seats_used / lic.seat_count) * 100))
    : null;
  const seatColor =
    seatPct === null
      ? ''
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                <span className="capitalize">{lic.license_type}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-2 space-y-4">
            ...
            {lic.seat_count ? (
              <>
                ...
                <p className="text-xs text-muted-foreground">
                  {seatPct}% seats in use
                </p>
              </>
            ) : (
              <p className="text-sm text-muted-foreground">
                Unlimited seats · {lic.seats_used} currently assigned
              </p>
            )}
          </div>
          ...
          <div className="rounded-xl border border-border bg-card p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field
                label="License Type"
                value={<span className="capitalize">{lic.license_type}</span>}
              />
              <Field
                label="Auto-renews"
                value={lic.auto_renews ? 'Yes' : 'No'}
              />
              ...
              <Field
                label="Annual Cost"
                value={fmtMoney(lic.cost_per_year, lic.currency)}
              />
              {lic.vendor_contact && (
                <Field label="Vendor Contact" value={lic.vendor_contact} />
              )}
              {lic.support_url && (
                <div>
