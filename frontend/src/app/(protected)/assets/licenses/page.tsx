...
export default function LicensesPage() {
  ...
  const { data, isLoading, mutate } = useSWR<{
    ...
    total: number;
    pages: number;
  }>(orgId ? [`/api/assets/licenses?${params}`, orgId] : null, ([url, oid]) =>
    api.get(url, oid as string)
  );

  const licenses = (data?.licenses || []).filter(
    l => !typeFilter || l.license_type === typeFilter
  );
