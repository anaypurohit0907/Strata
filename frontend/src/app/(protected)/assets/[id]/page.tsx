  ...
  const lookup = async () => {
    ...
    try {
      const res = await api.get<{ user_id: string; email: string }>(
        `/api/organizations/lookup-user?email=${encodeURIComponent(email)}`,
        orgId
      );
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-card border border-border rounded-xl p-6 w-full max-w-sm space-y-4">
        ...
        {err && (
          <p className="text-xs text-red-400 p-2 bg-red-500/10 rounded-lg">
            {err}
          </p>
        )}
        <div>
          ...
          {userId && (
            <p className="text-xs text-emerald-400 mt-1">✓ User found</p>
          )}
        </div>
        <div>
          <label className="block text-xs text-muted-foreground mb-1">
...
function RepairModal({
  ...
  const submit = async () => {
    if (!form.sent_date) {
      setErr('Sent date required');
      return;
    }
    setSaving(true);
    try {
      await api.post(
        `/api/assets/${assetId}/repairs`,
        {
          ...form,
          repair_cost: form.repair_cost
            ? parseFloat(form.repair_cost)
            : undefined,
        },
        orgId
      );
      ...
      <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
        <div className="bg-card border border-border rounded-xl p-6 text-center space-y-3">
          <p className="text-sm text-muted-foreground">
            No valid transitions from <strong>{current}</strong>
          </p>
          <button
            onClick={onClose}
...
interface AssetLicenseAssignment {
  id: string;
...
interface AssetDetail {
  ...
  created_at?: string | null;
  repairs?: AssetRepair[];
  tickets?: AssetLinkedTicket[];
  licenses: AssetLicenseAssignment[];
}
...
export default function AssetDetailPage({
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR(orgId ? [`/api/assets/${id}`, orgId] : null, ([url, oid]) =>
    api.get<AssetDetail>(url, oid)
  );
  ...
  const handleUnassign = async () => {
    if (!orgId || !confirm('Unassign this asset? It will move to In Storage.'))
      return;
    setUnassigning(true);
    try {
  ...
  const warrantyDays = asset.warranty_expiry
    ? Math.floor(
        (new Date(asset.warranty_expiry).getTime() - Date.now()) / 86400000
      )
    : null;
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-card border border-border rounded-xl p-5">
                ...
                {asset.condition_rating && (
                  <span
                    className={cn(
                      ...
                      COND_COLOR[asset.condition_rating]
                    )}
                  >
                    {asset.condition_rating}
                  </span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <Field
                  label="Assigned To"
                  value={asset.assigned_name || asset.assigned_email}
                />
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <Field
                  ...
                  value={fmtMoney(asset.purchase_price, asset.currency)}
                />
                ...
                <div className="mt-4 p-3 rounded-lg bg-muted/30 border border-border flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold">
                      {fmtMoney(dep.current_value, asset.currency)}
                    </p>
                  </div>
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              {warrantyDays !== null && (
                ...
                >
                  {warrantyDays < 0 ? (
                    <>
                      <AlertTriangle className="w-4 h-4" /> Warranty expired{' '}
                      {Math.abs(warrantyDays)} days ago
                    </>
                  ) : warrantyDays <= 90 ? (
                    <>
                      <Clock className="w-4 h-4" /> Warranty expires in{' '}
                      {warrantyDays} days
                    </>
                  ) : (
                    <>
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-semibold">
                  Repairs ({asset.repairs?.length ?? 0})
                </h2>
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              <h2 className="text-sm font-semibold mb-4">
                Linked Tickets ({asset.tickets?.length ?? 0})
              </h2>
            ...
            {asset.licenses?.length > 0 && (
              <div className="bg-card border border-border rounded-xl p-5">
                <h2 className="text-sm font-semibold mb-4">
                  Software Licenses ({asset.licenses.length})
                </h2>
                <div className="space-y-2">
...
function ContractCoverageCard({
  ...
  const { data } = useSWR(
    ...
    ([url, oid]) =>
      api.get<{ contracts: AssetContract[]; total: number }>(url, oid)
  );
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <span
                    className={cn(
                      ...
                      CONTRACT_STATUS_COLOR[c.status] ??
                        'text-muted-foreground bg-muted/50 border-border'
                    )}
                  >
                    {c.status}
...
function HistoryFeed({ assetId, orgId }: { assetId: string; orgId: string }) {
  const { data } = useSWR(
    ...
    ([url, oid]) => api.get<{ history: AssetHistoryEntry[] }>(url, oid)
  );

  if (!data?.history?.length)
    return <p className="text-xs text-muted-foreground/60">No history yet</p>;
        ...
        <div key={h.id} className="flex items-start gap-2">
          ...
          <div className="min-w-0">
            <p className="text-xs text-foreground leading-snug">
              {h.note ||
                `${h.event_type.replace(/_/g, ' ')}${h.field_changed ? ` — ${h.field_changed}` : ''}`}
            </p>
