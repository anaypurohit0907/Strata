  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        <span>
          Priority: <strong>{SLA_LABELS[item.sla_priority] ?? 'Medium'}</strong>
        </span>
        {item.estimated_time && (
          <>
            <span>·</span>
        ...
        <span>·</span>
        <span>
          {item.form_schema.length} field
          {item.form_schema.length !== 1 ? 's' : ''}
        </span>
      </div>
    </div>
  );
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                value={form.sla_priority}
                onChange={e =>
                  setForm({ ...form, sla_priority: Number(e.target.value) })
                }
                className="mt-1 w-full text-sm bg-background border border-border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-ring"
              >
            ...
            <div>
              ...
              <input
                value={form.auto_assign_role}
                onChange={e =>
                  setForm({ ...form, auto_assign_role: e.target.value })
                }
          ...
          <div>
            ...
            {fields.length === 0 && (
              <p className="text-xs text-muted-foreground italic">
                No fields yet — requester name and email are always collected
                automatically.
              </p>
            )}
              ...
              {fields.map((f, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <input
                    ...
                    className="w-28 text-xs bg-background border border-border rounded-lg px-2 py-1.5 focus:outline-none font-mono"
                  />
                  <input
                    value={f.label}
...
export default function ServiceHubPage() {
  ...
  const { data, isLoading, mutate } = useSWR<{ catalog: CatalogItem[] }>(
    '/api/servicehub/catalog?include_inactive=true'
  );
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage your service catalog — {publicCount} public service
            {publicCount !== 1 ? 's' : ''} visible to employees
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center py-20 gap-3 text-center">
          ...
          <p className="text-sm text-muted-foreground max-w-sm">
            Add services employees can request — laptop setup, VPN access,
