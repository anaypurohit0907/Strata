  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <span
          className={`flex items-center gap-1.5 text-sm font-medium ${rc.color}`}
        >
          {rc.icon} {rc.label}
        </span>
      ...
      <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
        <div
          className={`h-full ${rc.bg} rounded-full transition-all`}
          style={{ width: `${score}%` }}
        />
      </div>
      {score >= 60 && (
...
function ContractRow({ c }: { c: Contract }) {
  ...
  return (
    ...
    >
      <FileText
        size={14}
        className={`shrink-0 ${expirySoon ? 'text-red-400' : expiryWarning ? 'text-amber-400' : 'text-emerald-400'}`}
      />
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-zinc-500">
          ...
          {expirySoon && (
            <span className="text-red-400 ml-1">(renew soon)</span>
          )}
        </p>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        ...
        <span
          className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded border ${st.color}`}
        >
          {st.icon}
...
export default function VendorDetailPage() {
  ...
  const { data: vendor, error } = useSWR<Vendor>(
    params.id ? `/api/contracts/vendors/${params.id}` : null,
    fetcher
  );
  const { data: analyticsData } = useSWR<{ analytics: Analytics }>(
    params.id ? `/api/contracts/vendors/${params.id}/analytics` : null,
    fetcher
  );
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-zinc-950 p-6">
      {editOpen && (
        <EditVendorModal vendor={vendor} onClose={() => setEditOpen(false)} />
      )}
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div>
          <Link
            href="/contracts"
            className="flex items-center gap-1 text-sm text-zinc-500 hover:text-white mb-4 transition-colors w-fit"
          >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {analytics && (
                    <span
                      className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border ${rc.bg10} ${rc.border} ${rc.color}`}
                    >
                      {rc.icon} {rc.label}
                    </span>
                  )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
            ...
            <p className="text-xs text-zinc-600 mt-0.5">
              {analytics
                ? `${analytics.active_contracts} active contract${analytics.active_contracts !== 1 ? 's' : ''}`
                : 'loading…'}
            </p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
            ...
            <p className="text-xs text-zinc-600 mt-0.5">
              {analytics
                ? `across ${analytics.total_contracts} contracts`
                : 'loading…'}
            </p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          ...
          <div
            className={`rounded-xl p-4 border ${riskLevel === 'critical' ? 'bg-red-950/20 border-red-500/20' : riskLevel === 'warning' ? 'bg-amber-950/20 border-amber-500/20' : 'bg-zinc-900 border-zinc-800'}`}
          >
            <div className="flex items-center gap-2 mb-1">
          ...
          <div className="flex items-start gap-3 bg-red-950/20 border border-red-500/30 rounded-xl px-4 py-3">
            ...
            <div>
              <p className="text-sm font-medium text-red-300">
                {analytics.critical_expiring} contract
                {analytics.critical_expiring !== 1 ? 's' : ''} expiring within
                30 days
              </p>
              {analytics.soonest_expiry && (
                <p className="text-xs text-red-400/70 mt-0.5">
                  Soonest: {fmtDate(analytics.soonest_expiry)}
                  {analytics.days_until_soonest_expiry != null &&
                  analytics.days_until_soonest_expiry >= 0
            ...
            {analytics && (
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-4">
                ...
                <div className="pt-2 border-t border-zinc-800 grid grid-cols-2 gap-y-2 text-xs">
                  ...
                  <span
                    className={`text-right ${analytics.expired_contracts > 0 ? 'text-red-400' : 'text-zinc-300'}`}
                  >
                    {analytics.expired_contracts}
                  </span>
                  <span className="text-zinc-500">Closed</span>
                  <span className="text-zinc-300 text-right">
                    {analytics.closed_contracts}
                  </span>
                  {analytics.avg_contract_value > 0 && (
                    <>
                      <span className="text-zinc-500">Avg value</span>
                      <span className="text-zinc-300 text-right">
                        {formatCurrency(analytics.avg_contract_value)}
                      </span>
                    </>
                  )}
                </div>
                {analytics.latest_expiry && (
                  <p className="text-xs text-zinc-500">
                    ...
                    <span className="text-zinc-300">
                      {fmtDate(analytics.latest_expiry)}
                    </span>
                  </p>
                )}
              </div>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-2">
                ...
                {vendor.account_manager_email && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <a
                      href={`mailto:${vendor.account_manager_email}`}
                ...
                {analytics && analytics.total_contracts > 0 && (
                  <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs text-zinc-400">
                        {analytics.active_contracts}/{analytics.total_contracts}{' '}
                        active
                      </span>
                    </div>
