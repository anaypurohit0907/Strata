...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <label className="block text-xs text-zinc-400 mb-1.5">{children}</label>
  );
}
...
export default function NewVendorPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      const res = await api.post<{ id: string }>('/api/contracts/vendors', {
        ...form,
      ...
      router.push(`/contracts/vendors/${res.id}`);
    } catch (e: unknown) {
      setError(errorMessage(e, 'Failed to create vendor'));
      setSaving(false);
    }
  }
          ...
          <Section title="Account Manager">
            <div>
              ...
              <Input
                ...
                onChange={v => set('account_manager', v)}
            ...
            <div>
              ...
              <Input
                ...
                onChange={v => set('account_manager_email', v)}
                type="email"
              />
            </div>
          </Section>
