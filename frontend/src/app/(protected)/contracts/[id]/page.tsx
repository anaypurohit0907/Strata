...
interface Contract {
  ...
  vendor_support_email: string | null;
  vendor_support_phone: string | null;
...
function ExpiryTimeline({ contract }: { contract: Contract }) {
  ...
  const noticePct = contract.renewal_notice_days
    ? Math.round(
        ((end - contract.renewal_notice_days * 86_400_000 - start) / total) *
          100
      )
    : null;

  const barColor = contract.is_expired
    ? 'bg-red-500'
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
      ...
      <div className="flex items-center justify-between text-xs text-zinc-500">
        <span>
          {fmtDate(contract.start_date, { month: 'short', year: 'numeric' })}
        </span>
        {noticePct != null && noticePct > 0 && noticePct < 100 && (
          <span className="text-amber-400/70">
            Notice{' '}
            {fmtDate(contract.renewal_date, { month: 'short', day: 'numeric' })}
          </span>
        )}
        <span>
          {fmtDate(contract.end_date, { month: 'short', year: 'numeric' })}
        </span>
      </div>
    </div>
  );
...
const KEY_TERM_LABELS: Record<
  string,
  { label: string; icon: React.ReactNode; fmt?: (v: unknown) => string }
> = {
  sla_response_hours: {
    label: 'SLA Response',
  ...
  uptime_guarantee: {
    label: 'Uptime',
    icon: <CheckCheck size={13} className="text-emerald-400" />,
    fmt: v => String(v),
  },
  support_hours: {
    label: 'Support Hours',
  ...
  notice_period_days: {
    label: 'Notice Period',
    icon: <Bell size={13} className="text-amber-400" />,
    fmt: v => `${v} days`,
  },
  penalty_clause: {
...
function KeyTermsCard({ keyTerms }: { keyTerms: Record<string, unknown> }) {
  const entries = Object.entries(keyTerms).filter(
    ([, v]) => v != null && v !== ''
  );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {entries.map(([k, v]) => {
          ...
          return (
            <div
              key={k}
              className="flex items-start gap-2.5 p-3 rounded-lg bg-zinc-800/50 border border-zinc-700/50"
            >
              {def?.icon ?? (
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <span className="text-sm text-zinc-200">
                {contract.payment_amount
                  ? fmtCurrency(contract.payment_amount, contract.currency)
                  : '—'}
                {' / '}
                {PAYMENT_LABELS[contract.payment_schedule] ??
                  contract.payment_schedule}
              </span>
            </div>
          </div>
        ...
        {annual != null && (
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-zinc-300">
              {fmtCurrency(annual, contract.currency)}/yr
            </span>
          </div>
        )}
        <div className="flex items-center justify-between">
...
function StatusChanger({ contract }: { contract: Contract }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span
          className={`w-1.5 h-1.5 rounded-full ${st.dot} ${saving ? 'animate-pulse' : ''}`}
        />
        {st.label}
        <ChevronDown size={12} />
      </button>
      {open && (
        <div className="absolute top-full mt-1.5 left-0 z-30 bg-zinc-800 border border-zinc-700 rounded-xl py-1.5 min-w-[150px] shadow-xl">
          <p className="px-3 py-1 text-xs text-zinc-500 font-medium uppercase tracking-wide">
            Change status
          </p>
          {STATUSES.map(s => {
            ...
            return (
              ...
              >
                ...
                {sc?.label}
                {s === contract.status && (
                  <CheckCircle2 size={12} className="ml-auto text-zinc-400" />
                )}
              </button>
            );
          })}
...
function RenewModal({
  ...
  async function handleRenew() {
    ...
    setError('');
    try {
      const res = await api.post<{ new_contract: { id: string } }>(
        `/api/contracts/${contract.id}/renew`,
        {}
      );
      onClose();
      router.push(`/contracts/${res.new_contract.id}`);
    } catch (e: unknown) {
      setError(errorMessage(e, 'Renewal failed'));
      setLoading(false);
    }
  }
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400">
              Create a renewal draft from this contract
            </p>
          </div>
        </div>
          ...
          <div className="flex justify-between">
            ...
            <span className="text-zinc-200">
              {contract.end_date
                ? fmtDate(
                    new Date(
                      new Date(contract.end_date).getTime() + 86400000
                    ).toISOString()
                  )
                : '—'}
            </span>
...
function EditContractModal({
  ...
  const [form, setForm] = useState({
    ...
    total_value:
      contract.total_value != null ? String(contract.total_value) : '',
    currency: contract.currency,
    payment_schedule: contract.payment_schedule ?? '',
    payment_amount:
      contract.payment_amount != null ? String(contract.payment_amount) : '',
  ...
  const inp =
    'w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-500/50';
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-5 grid grid-cols-2 gap-4">
          {error && (
            <p className="col-span-2 text-sm text-red-400 bg-red-950/30 border border-red-500/20 rounded-lg px-3 py-2">
              {error}
            </p>
          )}
...
function LinkAssetModal({
  ...
  const { data } = useSWR<{
    assets: { id: string; asset_tag: string; name: string; status: string }[];
  }>(
    search.length >= 2
      ? `/api/assets?search=${encodeURIComponent(search)}&limit=10`
      : null,
    fetcher
  );
  const results: {
  ...
  async function link(assetId: string) {
    ...
    setError('');
    try {
      await api.post(`/api/contracts/${contractId}/link-asset`, {
        asset_id: assetId,
      });
...
export default function ContractDetailPage() {
  ...
  const { data: contract, error } = useSWR<Contract>(
    params.id ? `/api/contracts/${params.id}` : null,
    fetcher
  );
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-zinc-950 p-6">
      ...
      {renewOpen && (
        <RenewModal contract={contract} onClose={() => setRenewOpen(false)} />
      )}
      ...
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        <Link
          href="/contracts"
          className="flex items-center gap-1 text-sm text-zinc-500 hover:text-white transition-colors w-fit"
        >
        ...
        {contract.expiry_status !== 'ok' && (
          ...
          >
            ...
            <div className="flex-1">
              <p
                className={`text-sm font-medium ${contract.is_expired ? 'text-red-300' : contract.expiry_status === 'critical' ? 'text-orange-300' : 'text-amber-300'}`}
              >
                {contract.is_expired
              ...
              <p className="text-xs mt-0.5 opacity-70">
                {contract.auto_renews
                  ? 'Auto-renews. Verify terms are still favourable.'
                  : 'Manual renewal required before the notice deadline.'}
              </p>
            </div>
            {canRenew && (
              <button
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  {
                    label: 'Renewal Date',
                    value: fmtDate(contract.renewal_date),
                  },
                  {
                    label: 'Notice Period',
                    value: `${contract.renewal_notice_days} days`,
                  },
                ].map(({ label, value }) => (
                  <div
                    key={label}
            ...
            {contract.vendor_name && (
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
                ...
                <Link
                  href={`/contracts/vendors/${contract.vendor_id}`}
                ...
                <div className="space-y-2">
                  {contract.vendor_support_email && (
                    <a
                      href={`mailto:${contract.vendor_support_email}`}
                    ...
                    {contract.history.map((h, idx) => {
                      ...
                      return (
                        <div key={h.id} className="flex gap-4 py-3 relative">
                          <div
                            className={`w-4 h-4 rounded-full shrink-0 mt-0.5 border-2 z-10 ${
                              isCreate
