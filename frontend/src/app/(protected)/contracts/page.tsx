...
function StatCard({
  ...
  return (
    <div
      className={`bg-zinc-900 border rounded-xl p-4 ${alert ? 'border-amber-500/30 bg-amber-950/10' : 'border-zinc-800'}`}
    >
      <p className="text-xs text-zinc-500 uppercase tracking-wide">{label}</p>
      <p className={`text-2xl font-bold mt-1 ${accent}`}>{value}</p>
      {sub && (
        <p
          className={`text-xs mt-0.5 ${alert ? 'text-amber-400' : 'text-zinc-500'}`}
        >
          {sub}
        </p>
      )}
...
function StatusBadge({
  ...
  async function change(s: string) {
    ...
    try {
      ...
      globalMutate(
        (key: string) =>
          typeof key === 'string' && key.startsWith('/api/contracts'),
        undefined,
        { revalidate: true }
      );
      onChanged?.();
  ...
  return (
    <div className="relative" onClick={e => e.preventDefault()}>
      ...
      >
        <span
          className={`w-1.5 h-1.5 rounded-full ${cfg.dot} ${saving ? 'animate-pulse' : ''}`}
        />
        {cfg.label}
        <ChevronDown size={10} />
      </button>
...
function ExpiryBar({
  ...
  if (days == null)
    return <span className="text-xs text-zinc-600">No end date</span>;
  if (days < 0)
  ...
  const color =
    days <= 30 ? 'bg-red-500' : days <= 90 ? 'bg-amber-500' : 'bg-emerald-500';
  return (
    <div className="flex items-center gap-2 min-w-0">
      ...
      <span
        className={`text-xs shrink-0 ${days <= 30 ? 'text-red-400 font-medium' : days <= 90 ? 'text-amber-400' : 'text-zinc-500'}`}
      >
        {days}d
      </span>
    </div>
...
function VendorCard({ vendor }: { vendor: Vendor }) {
  return (
    <Link
      href={`/contracts/vendors/${vendor.id}`}
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-3">
        ...
        <ChevronRight
          size={15}
          className="text-zinc-600 group-hover:text-zinc-300 shrink-0 transition-colors mt-0.5"
        />
      </div>
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs text-zinc-400">
          ...
          active · {vendor.total_contracts} total
        </span>
...
function ContractRow({
  ...
  return (
    <div
      className={`flex items-center gap-0 group transition-colors ${selected ? 'bg-emerald-950/20' : 'hover:bg-zinc-800/40'}`}
    >
      {/* Checkbox */}
      <button
        type="button"
...
function CalendarPanel({ orgId }: { orgId: string }) {
  const { data } = useSWR<CalendarData>(
    orgId ? '/api/contracts/calendar?months=4' : null,
    fetcher
  );
          ...
          <div key={ym}>
            ...
            <div className="space-y-1.5">
              {contracts.map(c => (
                ...
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full shrink-0 ${EXPIRY_DOT[c.expiry_status] ?? 'bg-zinc-500'}`}
                  />
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
      ...
      <div className="flex h-2 rounded-full overflow-hidden gap-0.5 mb-3">
        {breakdown.map(b => (
          <div
            key={b.label}
            className={`${b.color} rounded-full min-w-0`}
...
export default function ContractsPage() {
  ...
  const { data: dash } = useSWR<Dashboard>(
    orgId ? '/api/contracts/dashboard' : null,
    fetcher
  );
  ...
  const contractsKey =
    orgId && tab === 'contracts' ? `/api/contracts?${contractParams}` : null;
  ...
  const { data: vendorsData } = useSWR<{ vendors: Vendor[]; total: number }>(
    orgId && tab === 'vendors'
      ? `/api/contracts/vendors?${vendorParams}&limit=100`
      : null,
    fetcher
  );
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-zinc-400 mt-0.5">
                  Vendor directory · contract lifecycle · renewal tracking
                </p>
              </div>
              <div className="flex items-center gap-2">
                <Link
                  href="/contracts/vendors/new"
            ...
            {stats && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <StatCard
                  ...
                  accent={
                    stats.expiring_30d > 0 ? 'text-amber-400' : 'text-white'
                  }
              ...
              <div className="bg-amber-950/20 border border-amber-500/25 rounded-xl">
                <div className="flex items-center gap-2.5 px-4 py-3 border-b border-amber-500/15">
                  ...
                  <p className="text-sm font-medium text-amber-300">
                    {dash.expiring_soon.length} contract
                    {dash.expiring_soon.length !== 1 ? 's' : ''} expiring in the
                    next 30 days
                  </p>
                </div>
                <div className="px-4 py-2 flex flex-wrap gap-x-6 gap-y-1.5">
                  {dash.expiring_soon.map(c => (
                    ...
                    >
                      <span className="text-zinc-300 group-hover:text-white transition-colors truncate max-w-[200px]">
                        {c.title}
                      </span>
                      <span className="text-amber-400 font-medium text-xs shrink-0">
                  ...
                  <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
                    <div className="flex items-center gap-3 px-4 py-2.5 border-b border-zinc-800 bg-zinc-900/50">
                      <button
                        type="button"
                        onClick={toggleAll}
                        className="pl-0.5 text-zinc-600 hover:text-zinc-300"
                      >
                        {selected.size > 0 &&
                        selected.size === contracts.length ? (
                      ...
                      <span className="text-xs text-zinc-500 font-medium flex-1">
                        CONTRACT
                      </span>
                      <span className="text-xs text-zinc-500 font-medium w-28 hidden md:block">
                        EXPIRY
                      </span>
                      <span className="text-xs text-zinc-500 font-medium w-20 text-right hidden lg:block">
                        VALUE
                      </span>
                      <span className="text-xs text-zinc-500 font-medium w-6 hidden sm:block" />
                      <span className="text-xs text-zinc-500 font-medium w-24">
                        STATUS
                      </span>
                      <span className="w-5" />
                    </div>
                  ...
                  {contractsData.pages > 1 && (
                    <div className="flex items-center justify-center gap-2 mt-2">
                      ...
                      <span className="text-sm text-zinc-500">
                        Page {page} of {contractsData.pages}
                      </span>
                      <button
                        type="button"
                        disabled={page >= contractsData.pages}
                        onClick={() => setPage(p => p + 1)}
                        className="px-3 py-1.5 text-sm rounded-lg bg-zinc-800 text-zinc-300 disabled:opacity-40 hover:bg-zinc-700 transition-colors"
                      >
                        Next
