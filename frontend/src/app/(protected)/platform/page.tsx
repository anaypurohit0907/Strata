    ...
    >
      ...
      {isActive && (
        <div
          className={cn(
            'h-0.5 w-full',
            `bg-gradient-to-r from-transparent via-${textAccent.split('-')[1]}-400/60 to-transparent`
          )}
        />
      )}
        ...
        {isActive ? (
          statsLoading ? (
            <div className="flex gap-1.5 mt-auto pt-3">
              {[60, 44, 36].map(w => (
                <div
                  key={w}
                  className="h-5 rounded-full bg-muted/60 animate-pulse"
                  style={{ width: w }}
                />
              ))}
            </div>
          ...
          <p className="text-[11px] text-muted-foreground mt-auto pt-3">
            ...
            <span className="font-semibold text-foreground/80">
              {requiredPlan && PLAN_LABELS[requiredPlan]}
            </span>{' '}
            plan
          </p>
        )}
...
export default function PlatformPage() {
  ...
  const fetchStats = useCallback(async () => {
    const active = PLATFORM_MODULES.filter(
      m => can(m.feature) && m.statsEndpoint
    );
          ...
          >
            <div>
              <p className="font-semibold text-sm">
                Unlock the full Strata platform
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                Upgrade to Starter to unlock BillingVault, AssetLog,
                ContractVault, ProcureFlow, ServiceHub, and KnowBase.
              </p>
            </div>
            <button
              type="button"
