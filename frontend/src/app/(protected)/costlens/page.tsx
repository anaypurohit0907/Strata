...
function UtilBar({ pct }: { pct: number }) {
  const color =
    pct < 40 ? 'bg-red-500' : pct < 70 ? 'bg-amber-400' : 'bg-emerald-400';
  return (
...
export default function CostLensPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .get<CostSummary>('/api/costlens/summary', orgId)
      .then(setData)
      .catch(() => {})
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
      ...
      {idle_assets.length > 0 && (
        <Section
          title="Idle Assets (90+ days no activity)"
          icon={Package}
          count={idle_assets.length}
        >
          {idle_assets.map(a => (
      ...
      {upcoming_renewals.length > 0 && (
        <Section
          title="Renewals in Next 90 Days"
          icon={Clock}
          ...
          <Section title="Software Spend by Vendor" icon={DollarSign}>
            {vendor_spend.slice(0, 6).map(v => (
              <div key={v.vendor} className="px-5 py-3 flex items-center gap-3">
                ...
                <span className="text-xs text-muted-foreground shrink-0">
                  {v.license_count} licenses
                </span>
                <span className="text-sm font-semibold w-16 text-right shrink-0">
                  {fmt(v.annual_spend)}/yr
                </span>
              </div>
            ))}
          </Section>
        ...
        upcoming_renewals.length === 0 && (
          <div className="text-center py-16 text-muted-foreground">
            ...
            <p className="text-sm mt-1">
              Add assets, licenses and contracts to start seeing savings
              opportunities.
            </p>
          </div>
        )}
