    ...
    return (
      <PageShell>
        <div className="flex items-center justify-center py-24 text-destructive gap-2">
          <AlertCircle className="h-5 w-5" /> Invoice not found
        </div>
      </PageShell>
    );
    ...
    <FeatureGate feature="billing" requiredPlan="starter">
      <PageShell>
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="text-right">
                ...
                {amountDue > 0 && invoice.status !== 'paid' && (
                  ...
                  >
                    {fmtMoney(amountDue, invoice.currency)} due
            ...
            <div className="rounded-lg border bg-card p-4">
              ...
              {invoice.client_tax_id && (
                <p className="text-xs text-muted-foreground mt-1">
                  {invoice.client_tax_label ?? 'Tax ID'}:{' '}
                  {invoice.client_tax_id}
                </p>
              )}
            </div>
              ...
              <div className="space-y-1.5 text-sm min-w-[200px]">
                ...
                {+invoice.tax_total > 0 && (
                  <div className="flex justify-between gap-8 text-muted-foreground">
                    <span>Tax</span>
                    <span className="tabular-nums">
                      {fmtMoney(invoice.tax_total, invoice.currency)}
                    </span>
                  </div>
                )}
                ...
                {+invoice.amount_paid > 0 && (
                  <>
                    ...
                    <div className="flex justify-between gap-8 font-bold border-t pt-1.5">
                      ...
                      >
                        {fmtMoney(amountDue, invoice.currency)}
                      </span>
                    </div>
                  </>
                )}
                ...
                <tbody className="divide-y divide-border">
                  {invoice.payments.map(pay => (
                    <tr key={pay.id}>
                      ...
                      <td className="px-4 py-3 text-right font-medium text-emerald-400 tabular-nums">
                        {fmtMoney(pay.amount, invoice.currency)}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {showPayModal && (
          ...
          >
            <div
              className="bg-card rounded-xl border p-6 w-full max-w-md space-y-4"
              onClick={e => e.stopPropagation()}
            >
              <h3 className="font-bold text-lg">Record Payment</h3>
              <p className="text-sm text-muted-foreground">
                Balance due:{' '}
                <strong>{fmtMoney(amountDue, invoice.currency)}</strong>
              </p>
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder={fmtMoney(amountDue, invoice.currency).replace(
                      /[^0-9.]/g,
                      ''
                    )}
                  />
