...
export default function BillingPage() {
  ...
  return (
    <FeatureGate
      ...
      description="Create professional invoices, track payments, and manage client billing."
    >
      <PageShell>
        <m.div
          variants={v.fadeUp}
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <Button
                variant="outline"
                size="sm"
                onClick={() => router.push('/billing/clients')}
              >
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <Card
                  className={data?.overdue_count ? 'border-red-500/30' : ''}
                >
                  <CardContent className="pt-4 pb-4">
                    ...
                    <p
                      className={cn(
                        'text-xl font-bold',
                        data?.overdue_count ? 'text-red-400' : ''
                      )}
                    >
                      {fmtMoney(data?.overdue_amount ?? 0)}
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {data?.overdue_count ?? 0} invoices
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardContent className="pt-4 pb-4">
                    ...
                    <p className="text-xl font-bold">
                      {fmtMoney(data?.outstanding_amount ?? 0)}
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {data?.outstanding ?? 0} invoices
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardContent className="pt-4 pb-4">
                    ...
                    <p className="text-xl font-bold text-emerald-400">
                      {fmtMoney(data?.paid_this_month ?? 0)}
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {data?.paid_count ?? 0} total paid
                    </p>
                  </CardContent>
                </Card>
