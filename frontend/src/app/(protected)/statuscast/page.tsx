...
function ServiceRow({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-border bg-card">
      ...
      <span
        className={cn(
          'text-[10px] px-2 py-0.5 rounded-full border font-semibold hidden sm:block',
          cfg.badge
        )}
      >
        {cfg.label}
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={e =>
                setForm({ ...form, status_impact: e.target.value })
              }
              className="mt-1 w-full text-sm bg-background border border-border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-ring"
            >
...
export default function StatusCastPage() {
  ...
  } = useSWR<{ services: Service[] }>('/api/statuscast/services');
  const { data: statsData, mutate: mutateStats } = useSWR<Stats>(
    '/api/statuscast/platform-stats'
  );
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage your public status page
          </p>
        </div>
