...
function ResultCard({
  ...
  const ns = NS_CONFIG[card.namespace] ??
    NS_CONFIG[card.entity_type] ?? {
      icon: Search,
      color: 'text-muted-foreground',
      label: card.entity_type,
    };
  const Icon = ns.icon;
  return (
    <button
...
export function CommandBar() {
  ...
  const search = useCallback(async (q: string) => {
    ...
    setLoading(true);
    try {
      const data = await api.post<QueryResult>('/api/casper/query', {
        query: q,
        top_k: 5,
      });
  ...
  const trigger = (
    ...
    >
      ...
      <kbd className="hidden sm:block text-[10px] px-1.5 py-0.5 rounded bg-muted border border-border font-mono">
        ⌘K
      </kbd>
    </button>
  );
    ...
    <div className="fixed inset-0 bg-black/60 z-[9999] flex items-start justify-center pt-[15vh] px-4">
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="max-h-80 overflow-y-auto">
          ...
          {results.map(card => (
            <ResultCard
              key={`${card.namespace}-${card.entity_id}`}
              card={card}
              onNavigate={navigate}
            />
          ))}
        ...
        {results.length > 0 && (
          <div className="px-4 py-2 border-t border-border text-xs text-muted-foreground">
            {results.length} result{results.length !== 1 ? 's' : ''} · Powered
            by CASPER
          </div>
        )}
      </div>
